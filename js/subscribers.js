...
async function subscribe(userId) {
    ...
    // Перевірити, чи ви вже підписані на цього користувача
    const isSubscribed = currentUserSubscriptions.includes(String(userId));
...
document.addEventListener('DOMContentLoaded', async function () {
    try {
        // Отримати ідентифікатори користувачів, на яких ви підписані
        const currentUserSubscriptions = await getCurrentUserSubscriptions();

        // Перевірити, чи користувач підписаний
        // const isSubscribed = currentUserSubscriptions.some(userId => userId === loggedInUserId);
        const isSubscribed = currentUserSubscriptions.includes(loggedInUserId);
        console.log('isSubscribed', isSubscribed)
